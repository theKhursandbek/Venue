    ...
    <div className="min-h-dvh flex flex-col relative">
      ...
      <header className="sticky top-0 z-40 glass-strong">
        <div className="px-5 h-14 flex items-center justify-between">
          <NavLink to="/" className="flex items-center gap-2.5">
            ...
            <span className="text-[16px] font-bold text-surface-900 tracking-tight">Venue</span>
          </NavLink>
          {isAuthenticated && (
            <button
              onClick={handleLogout}
              className="size-9 rounded-xl glass flex items-center justify-center text-surface-500 hover:text-surface-700 transition-colors active:scale-90"
              title="Выйти"
            >
      ...
      {isAuthenticated && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-40 safe-area-bottom">
          <nav className="flex items-center gap-1 glass-strong rounded-2xl px-2 py-2 shadow-xl shadow-black/8">
